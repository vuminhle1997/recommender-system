...
if (fs.existsSync(path.join(process.cwd(), 'data', 'comments_id.csv'))) {
    fs.createReadStream(path.join(process.cwd(), 'data', 'comments_id.csv'))
        ...
        .on('end', () => {
            ...
            const tvseries = recommender.getTopNTVSeriesByMeanScores(100);
            const dampedTvSeries = recommender.getTopNTVSeriesByDampedMeanScores(100);
            ...
            for (const entry of dampedTvSeries) {
                for (const [movie, rating] of entry) {
                    obj.list.push(({movie, rating}))
                }

            }
            for (const entry of tvseries) {
                for (const [movie,rating] of entry) {
                    anotherObj.list.push(({movie, rating}))
                }
            }
            const string = JSON.stringify(obj);
            const anotherString = JSON.stringify(anotherObj);
            fs.writeFile(path.join(process.cwd(), 'data', 'damped.json'), string, (err) => {
                if (err) {
                    console.error(err);
                }
            })
            fs.writeFile(path.join(process.cwd(), 'data', 'normal.json'), anotherString, (err) => {
                if (err) {
                    console.error(err);
                }
            })
